...
function rangeQuery(range: TimeRange) {
  ...
  return query({ from: iso(from), to: iso(to), points: 300 });
}
...
function useSeries(key: SeriesKey, path: string) {
  ...
  return useQuery({
    queryKey: key,
    queryFn: () => api.get<MetricSeries>(`${path}${rangeQuery(range)}`),
...
/** Each temperature sensor over time, keyed `{device}/{sensor}`: the hottest reading in each point. */
export function useTemperatureHistory(id: string, range: TimeRange) {
  return useSeries(metricKeys.temperatures(id, range), `/api/hosts/${id}/temperatures`);
}

/** The temperature history of every host that reported temperatures in the range, by name. */
export function useFleetTemperatures(range: TimeRange) {
  return useQuery({
    queryKey: metricKeys.fleetTemperatures(range),
    queryFn: () => api.get<HostTemperatures[]>(`/api/hosts/temperatures${rangeQuery(range)}`),
